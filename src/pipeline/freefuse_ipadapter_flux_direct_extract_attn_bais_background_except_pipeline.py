...
class FreeFuseFluxIpAdapterDirectExtractAttnBaisBGExceptPipeline(
    DiffusionPipeline,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
    ...
    def __call__(
        ...
        def _run_denoising_step(
            ...
            # Set IP Adapter image embeds for conditional inference
        ...
        def _process_masks_and_debug(
            ...
            # === Old Background Exclusion Logic (Method 1: Top 50% Threshold) ===
            # Commented out for easy rollback
            # # For each concept, find the top 50% tokens by similarity value
            # # A token is "background" if it's NOT in the top 50% for ANY concept
            # B, C, N, _ = concept_sim_maps_tensor.shape
            # sim_map_squeezed = concept_sim_maps_tensor.squeeze(dim=-1)  # B, C, N
            # 
            # # For each concept, compute the threshold (50th percentile)
            # # and create a mask of top 50% tokens
            # top_50_masks = []
            # for c in range(C):
            #     concept_sim = sim_map_squeezed[:, c, :]  # B, N
            #     # Compute 50th percentile threshold for each batch
            #     threshold = torch.quantile(concept_sim.float(), 0.6, dim=-1, keepdim=True)  # B, 1
            #     # Mask: True if token is in top 50% (above or equal to threshold)
            #     top_50_mask = concept_sim >= threshold  # B, N
            #     top_50_masks.append(top_50_mask)
            # 
            # # Stack to get B, C, N
            # top_50_masks = torch.stack(top_50_masks, dim=1)  # B, C, N
            # 
            # # A token is foreground if it's in top 50% of at least one concept
            # # not_background_mask: B, N - True if token is foreground (not background)
            # not_background_mask = top_50_masks.any(dim=1)  # B, N
            # 
            # # max_indices = concept_sim_maps_tensor.argmax(dim=1)  # B H*W
            # max_indices = self.stabilized_balanced_argmax_v3(sim_map_squeezed, height // 16, width // 16)  # B H*W
            #
            # for idx, lora_name in enumerate(concept_sim_maps.keys()):
            #     # Original mask from argmax
            #     argmax_mask = (max_indices == idx).float()  # B, N
            #     # Intersect with foreground mask: only keep tokens that are not background
            #     lora_masks[lora_name] = argmax_mask * not_background_mask.float()
            ...
            if debug_save_path is not None:
                ...
                # Visualize lora_masks (after background exclusion) using 'cividis' colormap
                for lora_name, mask in lora_masks.items():
                    mask_2d = mask[0].view(latent_h, latent_w).cpu().float().numpy()
                    fig, ax = plt.subplots(figsize=(8, 8))
                    im = ax.imshow(mask_2d, cmap='cividis')
                    ax.set_title(f'LoRA Mask (After BG Exclusion): {lora_name} (step {step_idx})')
                    plt.colorbar(im, ax=ax)
                    plt.savefig(os.path.join(debug_save_path, f'lora_mask_{lora_name}_step{step_idx}.png'), dpi=150, bbox_inches='tight')
                    plt.close(fig)

                # Visualize concept_sim_maps using 'viridis' colormap
